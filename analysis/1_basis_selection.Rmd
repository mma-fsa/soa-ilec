...
df_exp_basis <- tbl_ilec %>% 
  ...
  mutate(
    Actual_Value = case_when(
      stringr::str_ends(string = Expected_Basis_Name, pattern = "by_Amount") ~ Death_Claim_Amount,
      stringr::str_ends(string = Expected_Basis_Name, pattern = "by_Policy") ~ Number_Of_Deaths,
      TRUE ~ NA
    ),
    LL = dpois(
      Actual_Value, Expected_Basis_Value, log=T
    )
  ) %>%
  group_by(
    Expected_Basis_Name,
